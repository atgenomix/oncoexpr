...
#' @export
#' 
kegg_enrich_dotplot <- function(gene_list_, save_path_=NULL, save_filename_=NULL, showCategory_ = 10, enrichment_db = "org.Hs.eg.db"){
  save_parameter <- paste0(save_path_, save_filename_)
  gene_ids <- bitr(gene_list_, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = enrichment_db)
  organism <- if (identical(enrichment_db, "org.Mm.eg.db")) {
    "mmu"
  } else {
    "hsa"
  }
